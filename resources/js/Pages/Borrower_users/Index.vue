...
import
SearchUser from "@/Components/SearchUser.vue";
...
const deleteUser = async () => {
    try {
        await form.delete(route('Borrower_users.destroy', userToDelete.value.id));
    ...
    <AuthenticatedLayout>
        <template #header>
            ...
            <div class="flex justify-between items-center">
              ...
              <SearchUser v-model:search="searchTerm" @search="handleSearch" class="ml-auto" />
            </div>
          </template>
            ...
            <div class="overflow-hidden mb-8 w-full rounded-lg border shadow-xs">
                <div class="overflow-x-auto w-full">
                    <table class="w-full whitespace-no-wrap">
                        ...
                        <tbody>
                            <tr v-for="user in filteredUsers" :key="user.id" class="text-gray-700">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <span
                                        :class="{ 'text-green-500': user.status === 'activo', 'text-red-500': user.status === 'inactivo' }">
                                        {{ user.status }}
                                    </span>
                                </td>

                                <td class="px-4 py-3 text-sm">
                                    <div v-if="user.status === 'activo'" class="flex space-x-2">
                                        <NavLink :href="route('Borrower_users.show', user.id)">
                                            <ShowButton>Info</ShowButton>
                                        </NavLink>
                                        <NavLink :href="route('Borrower_users.edit', user.id)">
                                            <EditButton>Editar</EditButton>
                                        </NavLink>
                                        <DeleteButton @click="openModalDel(user)">Inactivar</DeleteButton>
                                    </div>
                                    <div v-else>
                                        <GreenButton @click="activateUser(user)">Reactivar</GreenButton>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
